...
void MyCoordinateSystem(Real x1, Real x2, Real x3, ParameterInput *pin,
  ...
  Real K0 = pin->GetReal("coord", "K0");
  Real K1 = pin->GetReal("coord", "K1");

  Real lx = xmax - xmin; 
  Real ly = ymax - ymin;
  Real cx = cos(TWO_PI*(x1 - xmin)/lx);
  Real sx = sin(TWO_PI*(x1 - xmin)/lx);
  Real cy = cos(TWO_PI*(x2 - ymin)/ly);
  Real sy = sin(TWO_PI*(x2 - ymin)/ly);
  ...
  g_inv(I00) = -1/(1 + K0*cx2);
  g_inv(I01) = 0.0;
  g_inv(I02) = 0.0;
  g_inv(I03) = 0.0;
  g_inv(I11) = (K1*cx2 - 3)/D;
  g_inv(I12) = K1*cx*cy/D;
  g_inv(I13) = 0.0;
  g_inv(I22) = (K1*cy2-3)/D;
  g_inv(I23) = 0.0;
  g_inv(I33) = 1.0;

  // Set x-derivatives of covariant components
  dg_dx1(I00) = -(2*TWO_PI/lx)*K0*cx*sx;
  dg_dx1(I01) = 0.0;
  dg_dx1(I02) = 0.0;
  dg_dx1(I03) = 0.0;
  dg_dx1(I11) = 0.0;
  dg_dx1(I12) = -(TWO_PI/(3*lx))*K1*sx*cy;
  dg_dx1(I13) = 0.0;
  dg_dx1(I22) = -(2*TWO_PI/(3*lx))*K1*cx*sx;
  dg_dx1(I23) = 0.0;
  dg_dx1(I33) = 0.0;
  ...
  dg_dx2(I02) = 0.0;
  dg_dx2(I03) = 0.0;
  dg_dx2(I11) = (2*TWO_PI/(3*ly))*K1*cy*sy;
  dg_dx2(I12) = -(TWO_PI/(3*ly))*K1*sy*cx;
  dg_dx2(I13) = 0.0;
  dg_dx2(I22) = 0.0;
  dg_dx2(I23) = 0.0;
  dg_dx2(I33) = 0.0;
